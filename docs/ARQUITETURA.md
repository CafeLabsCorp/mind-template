...
**Saber quando você está atrás:** o template versiona a base de engenharia num arquivo [`VERSION`](../VERSION) na raiz (inteiro, sobe quando `docs/`, `claude-user/`, `scripts/` ou `.claude/` mudam de um jeito que vale puxar) e descreve cada bump em [`CHANGELOG.md`](../CHANGELOG.md). Como o clone carrega o `VERSION` congelado no momento em que foi feito, `scripts/check-template-version.sh` compara o número local com o do template e avisa se você está atrás — **sem precisar de um clone local do template**: usa um `../mind-template/VERSION` irmão se houver, senão `git fetch upstream` + `upstream/main:VERSION`, senão pula com uma dica. A rodada de manutenção periódica (seção 13) roda essa checagem; ela só reporta, o `git merge upstream/main` continua sendo decisão sua.

---

## 13. Manutenção periódica do vault

**Decisão:** anotações acumulam histórico redundante com o tempo — tarefa concluída sem lição durável, fato duplicado entre nós, detalhe operacional que só importava no momento em que foi feito. Por isso vale ter uma rotina de manutenção periódica (cadência sugerida: semanal) que **sempre pergunta antes de rodar** — nunca edita o vault por conta própria sem confirmação da rodada. (Antes chamada "otimização"; renomeada porque a rodada faz mais que compactar.)
...
**Reconciliação com repositório complementar (se houver):** se o vault também alimenta, por dual-write, um repositório complementar de conhecimento compartilhado (ex.: uma base institucional de equipe/empresa, de dono coletivo, com vários vaults pessoais diferentes apontando pra ele), a mesma rodada também compara os nós equivalentes entre os dois e aponta divergência — sem essa checagem, uma atualização feita só de um lado nunca seria detectada, já que dual-write manual não tem sincronização automática.
